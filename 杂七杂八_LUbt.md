...
在而此时的 Neck 层中经过处理的特征就会像就像是金字塔一样，如下图所示：

![previewImag](https://p5-flow-imagex-sign.byteimg.com/isp-i18n-media/image/509d56d8545c7eeae2a795fe13506c8e~tplv-be4g95zd3a-image.jpeg?lk3s=8e244e95&rcl=202608101559461DE1CEABF4C5D6CE1A89&rrcfp=b2576990&x-expires=1794124793&x-signature=IhsXl35iXrGksuJIsJiYlY5NxuY%3D)
...
![](https://pic4.zhimg.com/v2-67a1b03d57f97f306eacca3db14d9e99_1440w.jpg)

因为 Yolo 就是通过不断的对图像进行卷积操作，并进行下采样，最后经过全连接层最后得到 一个 NxNxN 的输出，使用 YoloV1 来看的话，如下图：

[yolov1-v3](https://zhuanlan.zhihu.com/p/714048416)

![](https://pic3.zhimg.com/v2-1648efe25223c2c8bc7f1425b30c6ac4_1440w.jpg)

在这个模型中：

* 输入的图像大小为 448x448x3，在经过卷积层池化之后得到了 7x7x1024 的特征图；

* 然后将这个特征图经过两个全连接层之后输出 7x7x30 的输出，其中这个 7x7 就是论文中理解的网格的数量，而 30 为每个网格的特征值：其中 2个候选框(x, y, w, h, c) 共 10个，另外的 20 个则表示的类别数量，即每个类别的预测概率

> 也就是说这个特征图的数量实际上就是 <u>模型输出的尺寸</u>， 这个网格数量是能够通过卷积和池化的计算来调整的

## 为什么需要使用到网格的形式，并根据物体中心点落在了哪个网格单元格的内部，然后网格负责预测这个物体呢？这条逻辑是在什么的时候生效的呢？

直接回答： 这条逻辑是在模型训练的时候生效的

### 为什么输出 SxS 张量，逻辑上就对应了原图的 SxS 单元格？这个逻辑是哪里来的？

```
这个逻辑是 在训练阶段由损失函数 来进行决定的，即：
假设，当前这张图片的 标签的中心点 在 网格 的 （i， j） 中，并且标签的参数为（w，h， obj），那么在训练的过程中，当模型前向传播之后，输出的 （i，j）就要和这个 真实标签在网格 (i，j) 的真实标签来做一个损失计算，并开始反向传播
```

通过这个方式，来让模型的输出具有这个网格的含义
...
```
原图 ↓人为划分 S×S ↓物体中心落在哪个cell ↓那个cell承担这个物体的预测
```

论文原话就是：

> 如果一个物体的中心落在某个 grid cell 中，那么这个 grid cell 负责检测该物体。
...
#### “是不是损失函数的设计上，让他有了这些含义？”

对原始 YOLOv1 来说，可以这么理解。

更准确地说，是：

```
目标分配规则      ↓构造 training target      ↓损失函数比较 output 和 target      ↓反向传播      ↓output[i,j] 逐渐学会对应那个空间位置的任务
```

比如真实狗在 cell `(4,2)`：

```
target[4,2]=objectness = 1class = dogx = ...y = ...w = ...h = ...
```

loss 就要求：

```
prediction[4,2]        ↓尽量接近        ↓target[4,2]
```

训练了很多图片之后，网络就逐渐建立这种规律：

```
output[4,2]    ↓“这里的位置如果有东西，我应该报告它”
```

所以：

> **语义不是从网络结构“长出来”的，而是监督信号长期训练出来的。**



### 如果他要有这种空间关系的话，那如果我的目标物品所处于图片中的不同位置，那他不就不能识别出来了？这不是 bug 吗？

**有这个风险，但 CNN 的卷积权重共享帮助解决了它。**
...
**<u>神经网络输出的某个数字本身没有“意义”；意义来自我们怎样定义这个输出、怎样构造训练标签，以及怎样设计 loss。</u>**



#### 以前的那些 CNN 的预测框都是怎么生成的？

#### yolo 之所以快就是因为这个 网格机制？
...
3. 通过具体搞清除的这些内容来结合模型的表现来调整数据，比如说增加数据的多样性（泛化性）、域同步（不使用不同领域空间的，模型遇不到的数据，会让模型的学习特征冲淡，导致识别率不高），使用数据
...
[hash 是什么东西](https://blog.csdn.net/Beyond_2016/article/details/81286360)

哈希值实际上是由哈希函数的输出值，它具有以下的特点

| 特性       | 含义                     |
| -------- | ---------------------- |
| **固定长度** | 输入 1KB 还是 10GB，输出长度都一样 |
| **确定性**  | 同一个输入，永远得到同一个哈希值       |
| **雪崩效应** | 输入只改 1 个字节，输出结果天差地别    |
| **单向**   | 从哈希值几乎不可能反推出原始内容       |

也就是说，哈希值是一个唯一的指纹，但凡输入的内容有什么不同，那么他输出的固定长度的输出就会不一样

**一句话：哈希值是一段数据的 "数字指纹"。**

想象你有一台机器，你把任何东西扔进去 —— 一本书、一张照片、一段视频、哪怕只有一个字 —— 它都会吐出一个**固定长度**的字符串（比如 64 个十六进制字符）。这台机器叫**哈希函数**，吐出来的字符串就是**哈希值**。

其中哈希在图片文件中会有两种不同的含义：

1. 文件哈希（MD5/SHA-1/SHA-256）

**衡量：文件是否逐字节完全相同。**

- 对图片文件的**二进制内容**直接算哈希
- 只要文件有任何一个字节不同（改了一个像素、重新压缩、改了文件名元数据），哈希值就完全不同
- 用途：**文件完整性校验**、去重（精确去重）、防篡改

> 类比：DNA 鉴定。一个碱基对不同，就不是同一个人。

2. 感知哈希（aHash /dHash/pHash）    

**衡量：两张图片在视觉上有多像。**

这才是通常说 "图片哈希" 时更有意义的那种。它不看二进制，而是**提取图片的视觉特征**（亮度分布、频率结构、梯度变化等），压缩成一个短编码。

- 同一张图，哪怕你**裁剪、压缩、缩放、加滤镜、调亮度**，感知哈希值仍然很接近
- 两张完全不同的图，哈希值差异很大
- 用**汉明距离**（两个哈希值有多少位不同）来量化相似度：距离越小越像

> 类比："看脸认人"。同一个人换发型、戴眼镜、化了妆，你还是能认出来；但换个人就认不出了。

常见感知哈希对比：

| 类型          | 原理          | 速度  | 精度  | 适用   |
| ----------- | ----------- | --- | --- | ---- |
| aHash（均值哈希） | 比较像素与平均亮度   | 最快  | 一般  | 快速粗筛 |
| dHash（差异哈希） | 比较相邻像素梯度    | 快   | 较好  | 常用方案 |
| pHash（感知哈希） | DCT 变换取低频系数 | 较慢  | 最好  | 精确匹配 |

## 为什么要使用哈希表来进行查询一些东西，直接使用这个字符串比对不就好了，为什么还要使用哈希来进行给他生成编号，同时存储到哈希表中，然后到后面再使用这个编号来进行查询操作呢？

首先需要明确的是，哈希表实际上并不是 “使用哈希值来代替字符串进行比较”

在哈希表中，**哈希值并不是使用来做比较，而是使用于定位的**，找到位置之后，**还是要做一次真正的字符串比对**来确认。所以更准确的说法是：

> 哈希的作用是**把 "要在 100 万个字符串里找" 变成 "直接走到某个位置去找"**，把要做的比对次数从几十万次降到 1 次。

我们直接通过字符串来进行定位，那么在表中进行查找的时候，他的时间复杂度就是 $O(n)$

因为我们每个字符串都要重新进行读取比对之类的操作，

### 为什么要必须 “生成编号”— 因为内存只认数字地址

这一步是最深的本质，很多人没讲透：

**数组为什么能用下标一步找到元素？** 因为内存物理上就是按数字编号的（地址 0、1、2、3...）。你给我一个数字下标，CPU 用 "基地址 + 下标 × 元素大小" 算一下，一步就跳到那个位置。这是硬件层面的超能力，O (1)。

**但我们的问题是 "按内容找"，而内容（字符串）不是数字。** 你没法对硬件说 "我要找 0x 地址 处的 ' 张三 ' 这个字符串"—— 内存寻址只认数字。
...
**哈希函数就是架在断层上的桥**：它把任意内容翻译成一个数字，翻译出来的数字恰好落在数组下标范围内。于是内容成功 "变成" 了地址，你就能复用数组 O (1) 寻址的能力了。

> 数组寻址 O (1) 是因为 "你给我数字"；哈希表让 "你给字符串" 也能变成 "给你数字"。本质是把**查找问题**转化成了**地址计算问题**—— 查找要摸很多个，计算只要算一次。

上一轮讲的是**把哈希当 "指纹" 来比较**（两张图像不像、文件是否相同）。

这一轮哈希表是**把哈希当 "地址" 来定位**（该去哪个位置找）。

- 指纹用途：哈希值本身是比对对象
- 地址用途：哈希值只是指路牌，真正确认还是靠一次真实的字符串比对
...
# 为什么我们能够通过脚本的方式，来让 linux 系统的机器能够自动获取当前机器的 wifi 网卡所在的网段 ip ，并使用脚本来使用话这个网关地址，并将我们想要的 ip 地址重新与网关地址来进行改网关地址号
...
Linux 中的 tail 是一个用于访问文件的尾部内容的工具
...
他们是一个**大模型框架**，其中<u>大模型在这个框架里面相当于大脑</u>，<u>而 hermes 和 小龙虾相当于是身体</u>，能够让大模型能够自行去调用工具并完成我们下发给他的任务，直到完成为止

其中这两个框架各有相应的特点：

* openclaw：主打 网关 （Gateway）优先，typescript 开发

* Hermes agent：Python 开发，主打 **自主进化学习闭环**

## 为什么他们能够执行一个具体任务直到完成？

核心就是：Agent Loop (智能体循环，改良版的 ReAct)，这是框架的代码来实现的死循环，而不是模型自己循环来进行任务

```
while(没到达最大迭代次数):
    1、框架把用户指令 + 历史步骤 + 工具返回结果 打包喂给大模型
    2、大模型输出：要么【最终回答】，要么【tool_calls工具调用指令】
    3、如果是工具调用：框架本地执行工具（shell、搜索、读写文件），拿到执行结果
    4、把工具执行结果塞回对话上下文，再次请求大模型
    5、如果大模型不再输出tool_calls，输出最终文本 → 循环终止，任务结束
```

其中大模型就只是一个大脑，他只做思考决策，调用工具，任务工作流规划；循环调度、工具执行、状态保存全部是框架代码完成

## Hermes 所谓的 “自我进化” 是什么？

⚠️**不是模型权重训练 / 微调，不是 AI 产生新智慧，是代码层面的经验沉淀机制**。

1. 正常完成一轮复杂任务之后，Hermes 启动后台复盘子 Agent（还是调用同一个大模型）
2. 让大模型阅读完整任务日志，提炼这套任务的执行步骤，生成一份 Markdown 格式 Skill 技能文件保存到本地磁盘
3. 下次遇到相似需求，框架自动检索加载这份 Skill，直接复用这套步骤，减少重复思考、减少工具调用轮次火山引擎开...。

> 本质：**自动写 SOP 存本地，下次复用，没有修改大模型本身权重**。

- Hermes 自带一批预制免费 Skill，不是自带免费 Agent 模型；模型仍然需要你自己配置 API‑key 或者本地模型。
- 进化效果好坏完全依赖外接 LLM 的推理质量，模型差，提炼出来的 Skill 也会错。
